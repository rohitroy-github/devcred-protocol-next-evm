...
export default function JobDetailsPage({ params }) {
  ...
  async function runAction(action) {
    ...
    try {
      ...
      if (action === "assign") {
        await assignDeveloperOnChain(job.jobId, developerInput);
      }

      if (action === "submit") {
        await submitWorkOnChain(job.jobId);
      }

      if (action === "approve") {
        await approveWorkOnChain(job.jobId);
      }

      if (action === "cancel") {
        const cancelResult = await cancelJobOnChain(job.jobId);
        await fetch(`/api/jobs/${job.jobId}/actions`, {
          method: "POST",
